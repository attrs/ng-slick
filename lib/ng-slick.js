    ...
    link: function (scope, element, attrs) {
      ...
      .on('lazyLoaded', function(event, slick, image, src) {
        if( attrs.ngSlickLazyLoaded ) 
          scope.$parent.$eval(attrs.ngSlickLazyLoaded, {$event: event, $slick: slick, $image: image, $src: src});
      })
      .on('lazyLoadError', function(event, slick, direction) {
        if( attrs.ngSlickLazyLoadError ) 
          scope.$parent.$eval(attrs.ngSlickLazyLoadError, {$event: event, $slick: slick, $image: image, $src: src});
      });
